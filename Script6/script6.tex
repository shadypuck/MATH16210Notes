...
\begin{lemma}\label{lem:6.6}\marginnote{\emph{1/14:}}
    ...
    \begin{proof}
        Let $X$ be an arbitrary nonempty subset of $\R$ that is bounded above. To prove that $\sup X$ exists, we will show that $\sup X=\bigcup\{Y\mid Y\in X\}$. To show this, Definition \ref{dfn:5.7} tells us that it will suffice to demonstrate that $U=\bigcup\{Y\mid Y\in X\}$ is an upper bound of $X$ and if $U'$ is an upper bound of $X$, then $U\leq U'$. Let's begin.\par
        To demonstrate that $U$ is an upper bound of $X$, Definition \ref{dfn:5.6} tells us that it will suffice to confirm that $Y\leq U$ for all $Y\in X$. To confirm this, Definition \ref{dfn:6.4} tells us that it will suffice to verify that $Y\subset U$ for all $Y\in X$. But by Theorem \ref{trm:1.7b}, this is true.\par
