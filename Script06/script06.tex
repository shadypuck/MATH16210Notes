...
\begin{exercise}\label{exr:6.16}
    ...
    \begin{proof}[Proof of Exercise \ref{exr:6.16}]
        ...
        % Before we show that $f$ is surjective, we will first verify the following claim (which we will refer to as Lemma (d) for future reference): Consider the set $\{f(q_1),\dots,f(q_n)\}\subset K$; if the symbols $b_1,\dots,b_n$ are assigned to $f(q_1),\dots,f(q_n)$ such that $b_1<_Kb_2<_K\cdots<_Kb_n$, then $f^{-1}(b_1)<_\Q f^{-1}(b_2)<_\Q\cdots<_\Q f^{-1}(b_n)$. Let's begin. By Theorem \ref{trm:3.5}, the symbols $a_1,\dots,a_n$ can be assigned to $q_1,\dots,q_n$ so that $a_1<_\Q a_2<_\Q\cdots<_\Q a_n$. It follows by Lemma (c) that $f(a_1)<_Kf(a_2)<_K<\cdots<_Kf(a_n)$. Thus, if we let $b_i=f(a_i)$, we know that $a_1=f^{-1}(b_1)<_\Q a_2=f^{-1}(b_2)<_\Q\cdots<_\Q a_n=f^{-1}(b_n)$.\par
