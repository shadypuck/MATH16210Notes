...
\begin{lemma}\label{lem:8.3}
    ...
    \begin{proof}
        % We will prove the claim by contrapositive in the forward direction, and directly in the reverse direction. Let's begin.\par\medskip
        % Suppose first that $I\subsetneq\R$ does not take one of the eight forms in Equations \ref{eqn:8.1}. To prove that $I$ is not an interval, Definitions \ref{dfn:8.2} and \ref{dfn:1.3} tell us that it will suffice to show that for some $x,y\in I$ with $x<y$, there exists a point $z\in[x,y]$ such that $z\notin I$.

        % Let $I\subsetneq\R$ be an interval. Since $I\subsetneq\R$, we have by Definition \ref{dfn:1.3} that there exists a point $p\in\R$ such that $p\notin I$.
        
        % Since it is a proper subset, it is either bounded below or above (if unbounded, then it is not a proper subset). We divide into three cases (only bounded below, only bounded above, and bounded below and above). Suppose it's only bounded below. Then it has a greatest lower bound. If the greatest lower bound is in the set, then it's $[a,\infty)$. If not, then it's $(a,\infty)$. Symmetric if only bounded above. If bounded above and below, divide into four cases (inf in, sup in; inf in, sup not; inf not, sup in; inf not, sup not).
        % Suppose first that $I\subsetneq\R$ is an interval, and suppose for the sake of contradiction that $I$ does not take one of the eight forms in Equations \ref{eqn:8.1}.
